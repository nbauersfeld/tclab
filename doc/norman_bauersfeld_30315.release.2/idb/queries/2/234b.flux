...
tstart = -90d
measurement = "DS_SS23_TrayStates_B"

// lorry number with pack signal
pack = from(bucket: "factory2")
|> range(start: tstart)
|> filter(fn: (r) => ((r._measurement == measurement) and (r.Signal == "KameraP" and r.Assembly == "Packaging") and (r._value != 0)))
|> keep(columns: ["_time","_value"])
|> toInt()
|> set(key: "_field", value: "lorry")

// batch signal
batch = from(bucket: "factory2")
|> range(start: tstart)
|> filter(fn: (r) => ((r._measurement == measurement) and (r.Signal == "10B3" and r.Assembly == "Conveyor")))
|> keep(columns: ["_time","_value"])
|> toInt()
...
// flag loading
|> map(fn: (r) => ({ r with "_loading": if (4 != r._batch) and (-1 != r._batch) then 1 else 0 }))
// build tables
